class Node { //node we have data, and next for reference to next act as pointer property
    constructor(data) {
      this.data = data;
      this.next = null;
    }
  ...
  class LinkedList {
    ...
    delete(data){
        if(!this._head){ //if there is no head ,return null
            return null
        }
        if(this._head.data===data){  //checks if head contains data to be deleted, if yes
            //it sets to the next  node ,removing it 
            this._head = this._head.next;
            return;
        }
        let current = this._head;
        while (current.next) {  //if head does not contain data to be removed   enters into next in if statement
